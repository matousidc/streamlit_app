...
def get_price(coin: str, vs_coin: str) -> str:
    ...
    """
    params = {"ids": coin, "vs_currencies": vs_coin, "include_market_cap": 'true', "include_24hr_change": 'true'}
...
def coin_names():  # TODO: save coins into pickle, api time limit, check like once a day
    ...
    resp_vscoins = requests.get("https://api.coingecko.com/api/v3/simple/supported_vs_currencies")
    # print(len(resp.json()))
    # print(len(r.json()))
...
@st.cache
def coin_history(coin: str, vs_coin: str, days_back: int, interval: int | None = None) -> pd.DataFrame:
    """
    ...
    r = requests.get(f"https://api.coingecko.com/api/v3/coins/{coin}/market_chart", params=params)
...
def streamlit_options(coins: list[dict], vs_coins: list) -> tuple[str, str, int]:
    with st.form("my_form"):  # form batching choices together, app is rerun after clicking on submit
        option = st.selectbox(
            'What coin history do you want to see?',
            tuple(f"{x['name']}-${x['symbol']}" for x in coins))
        vs_option = st.selectbox(
            'Against which coin?',
            tuple(vs_coins))
        interval = st.radio(
            "How far back do you want to display the history?",
            ('1 day', '1 week', '1 month', '3 months'))
        interval_match = {x: y for x, y in zip(('1 day', '1 week', '1 month', '3 months'), (1, 7, 30, 90))}
